...
static uint64_t get_legal_moves(uint32_t *game_state, uint16_t *game_condition, uint16_t loc,
...
void update_legal_moves(uint32_t *game_state, uint16_t *game_condition, uint64_t *legal_moves,
                        uint32_t *prev_game_state, uint16_t color_to_move) {
...
uint16_t check_valid_move(uint32_t *game_state, uint16_t *game_condition, uint64_t *legal_moves, int32_t loc,
                          int32_t dest) {
...
uint16_t make_move(uint32_t *game_state, uint16_t *game_condition, uint64_t *legal_moves, int32_t *selected_square,
                   int32_t *clicked_square, uint16_t *color_to_move, uint32_t *prev_game_state) {
    ...
    if (check_valid_move(game_state, game_condition, legal_moves, *selected_square, *clicked_square)) {
        ...
        if (selected_piece == WHITE_KING && abs(*clicked_square - *selected_square) == 2 ||
            selected_piece == BLACK_KING && abs(*clicked_square - *selected_square) == 2) {
        ...
        } else if (selected_piece == WHITE_PAWN && *selected_square / BOARD_ROWS == FIRST_ROW + 1 &&
                   *clicked_square / BOARD_ROWS == FIRST_ROW) {
            do_pawn_promotion(game_state, *selected_square, *clicked_square);
        } else if (selected_piece == BLACK_PAWN && *selected_square / BOARD_ROWS == LAST_ROW - 1 &&
        ...
        if (selected_piece == WHITE_ROOK || selected_piece == BLACK_ROOK || selected_piece == WHITE_KING ||
