...
static void do_en_passant(uint32_t *game_state, uint16_t loc, uint16_t dest,
                          uint16_t color) {
  printf("ISDJFIOSD");
  switch (color) {
  case WHITE:
...
static void do_pawn_promotion(uint32_t *game_state, uint16_t loc,
                              uint16_t dest) {
  char line[100];
  char value;
...
uint32_t check_game_condition(uint16_t *game_condition, uint16_t piece,
                              int32_t location) {
  switch (piece) {
  case WHITE_ROOK:
...
void set_piece_moved(uint16_t *game_condition, uint16_t piece,
                     int32_t location) {
  switch (piece) {
  case WHITE_ROOK:
    if (location == LEFT_WHITE_ROOK) {
...
int16_t piece_in_direction(uint32_t *game_state, int32_t loc,
                           Direction direction) {
  do {
    loc += direction;
    if (loc < 0 || loc >= BOARD_SQUARES) {
...
static uint64_t get_legal_moves(uint32_t *game_state, uint16_t *game_condition,
                                uint16_t loc, uint32_t *prev_game_state) {
...
void update_legal_moves(uint32_t *game_state, uint16_t *game_condition,
                        uint64_t *legal_moves, uint32_t *prev_game_state,
                        uint16_t color_to_move) {
  for (int32_t i = 0; i < BOARD_SQUARES; i++) {
    legal_moves[i] =
        get_legal_moves(game_state, game_condition, i, prev_game_state);
  }
...
uint16_t check_valid_move(uint32_t *game_state, uint16_t *game_condition,
                          uint64_t *legal_moves, int32_t loc, int32_t dest) {
...
uint16_t make_move(uint32_t *game_state, uint16_t *game_condition,
                   uint64_t *legal_moves, int32_t *selected_square,
                   int32_t *clicked_square, uint16_t *color_to_move,
                   uint32_t *prev_game_state) {
  ...
  if (*selected_square == UNSELECTED) {
    if (clicked_piece != WHITE_EMPTY && clicked_piece != BLACK_EMPTY &&
        (clicked_piece & COLOR_BIT_MASK) == *color_to_move) {
  ...
  if (clicked_piece != WHITE_EMPTY && clicked_piece != BLACK_EMPTY &&
      (selected_piece & COLOR_BIT_MASK) == (clicked_piece & COLOR_BIT_MASK)) {
    *selected_square = *clicked_square;
    return MOVE_FAIL;
  }

  if (check_valid_move(game_state, game_condition, legal_moves,
                       *selected_square, *clicked_square)) {
    memcpy(prev_game_state, game_state, BOARD_ROWS * sizeof(int));
    if (selected_piece == WHITE_KING &&
            abs(*clicked_square - *selected_square) == 2 ||
        selected_piece == BLACK_KING &&
            abs(*clicked_square - *selected_square) == 2) {
      do_castles(game_state, *selected_square, *clicked_square);
    } else if (selected_piece == WHITE_PAWN &&
               (*clicked_square == *selected_square + NORTH_WEST ||
                *clicked_square == *selected_square + NORTH_EAST) &&
               (clicked_piece == WHITE_EMPTY || clicked_piece == BLACK_EMPTY)) {
      do_en_passant(game_state, *selected_square, *clicked_square, WHITE);
    } else if (selected_piece == BLACK_PAWN &&
               (*clicked_square == *selected_square + SOUTH_WEST ||
                *clicked_square == *selected_square + SOUTH_EAST) &&
               (clicked_piece == WHITE_EMPTY || clicked_piece == BLACK_EMPTY)) {
      do_en_passant(game_state, *selected_square, *clicked_square, BLACK);
    } else if (selected_piece == WHITE_PAWN &&
               *selected_square / BOARD_ROWS == FIRST_ROW + 1 &&
               *clicked_square / BOARD_ROWS == FIRST_ROW) {
      do_pawn_promotion(game_state, *selected_square, *clicked_square);
    } else if (selected_piece == BLACK_PAWN &&
               *selected_square / BOARD_ROWS == LAST_ROW - 1 &&
               *clicked_square / BOARD_ROWS == LAST_ROW) {
      do_pawn_promotion(game_state, *selected_square, *clicked_square);
    } else {
      move_piece(game_state, *selected_square, *clicked_square);
    }

    if (selected_piece == WHITE_ROOK || selected_piece == BLACK_ROOK ||
        selected_piece == WHITE_KING || selected_piece == BLACK_KING) {
      set_piece_moved(game_condition, selected_piece, *selected_square);
    }

    update_legal_moves(game_state, game_condition, legal_moves, prev_game_state,
                       *color_to_move);
